...
@Tag(
        name = "CRUD REST APIs for Cards in our fucking bank MyBank",
...
public class CardsController {
    ...
    @Operation(
            ...
            description = "REST API to create new Card inside MyBank based on a mobile number"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public ResponseEntity<CardsDto> fetchCardDetails(@RequestHeader("mybank-correlation-id") String correlationId,
                                                     ...
                                                     @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                     String mobileNumber) {
        logger.debug("fetchCardDetails method start right now baby !");
        CardsDto cardsDto = iCardsService.fetchCard(mobileNumber);
        logger.debug("fetchCardDetails method end right now baby !");
