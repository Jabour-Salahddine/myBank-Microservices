...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler { // this extends is just for handling validation issue properly, by default if we have a problem of validation it will be detected by our method :  handleGlobalException(Exception exception,  WebRequest webRequest)
    ...
    @ExceptionHandler(CustomerAlreadyExistsException.class) // This annotation tells Spring to handle exceptions of type CustomerAlreadyExistsException
    ...
    /*
    *  Sans cette methode pour les endpoints qui ont l'annotation (@Valid et aussi @requestBody) il seront gérés automatiquement par Spring, mais si on veut personnaliser la réponse de validation, on peut utiliser cette méthode :
    *  Pour les endpoints qui n'ont pas l'annotation @Valid, cette méthode ne sera pas appelée, et les erreurs de validation seront gérées par la méthode handleGlobalException(Exception exception,  WebRequest webRequest)
    * */
